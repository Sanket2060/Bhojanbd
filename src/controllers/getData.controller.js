    ...
    res
      ...
      .json(
        new ApiResponse(
          200,
          { topTenDonators },
          "Sent top ten donors successfully"
        )
      );
  } catch (error) {
    ...
    res
      ...
      .json(
        new ApiResponse(
          200,
          { topTenDistributors },
          "Sent top ten distributors successfully"
        )
      );
  } catch (error) {
    console.log("Error in retrieving top donors:", error);
...
const getUserDetailsFromName = asyncHandler(async function (req, res) {
  const { name } = req.body;
  const user =
    (await Donor.findOne({ name }).select("-password -refreshToken")) ||
    (await Distributor.findOne({ name }).select("-password -refreshToken"));
  if (!user) {
  ...
  res
    .status(200)
    .json(
      new ApiResponse(200, { user }, "Data of user extracted from name sent")
    );
});
  ...
  return res
    ...
    .json(
      new ApiResponse(
        200,
        result ? { result } : null,
        "Active Orders sent successfully"
      )
    );
});
    ...
    res
      ...
      .json(
        new ApiResponse(
          200,
          { completedOrders },
          "Completed orders for donor sent successfully"
        )
      );
  } catch (error) {
    console.log("Error in retrieving completed orders for donor:", error);
...
const getOrganizationDetails = asyncHandler(async (req, res) => {
  ...
  res
    .status(200)
    .json(
      new ApiResponse(200, { bhojan }, "Sent organization details successfully")
    );
});
...
const getDistributorsRank = asyncHandler(async (req, res) => {
  try {
    console.log(req);
    ...
    let rank;
    topTenDistributors.map((user, index) => {
    ...
    res
      .status(200)
      .json(
        new ApiResponse(200, { rank }, "Sent rank of distributor successfully")
      );
  } catch (error) {
    console.log("Error in retrieving distributors rank:", error);
  }
});
const getDonorsRank = asyncHandler(async (req, res) => {
  let rank;
  try {
    ...
    res
      .status(200)
      .json(new ApiResponse(200, { rank }, "Sent rank of donors successfully"));
  } catch (error) {
    console.log("Error in retrieving donors rank:", error);
  }
});
