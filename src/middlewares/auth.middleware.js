...
try {
       ...
       //find user from token on database to verify
        let   user=await Donor.findById(decodedToken?._id).select("-password -refreshToken") ||
    ...
    if (error=='TokenExpiredError: jwt expired'){
        ...
        const decodedToken=jwt.verify(refToken,process.env.REFRESH_TOKEN_SECRET); //the decoded token has all data sent during encoding of token
        //find user from token on database to verify
